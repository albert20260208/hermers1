...
# 已修复：添加 --break-system-packages 支持 Ubuntu 24.04/Python 3.12+
#
...
# 安装核心依赖（添加 --break-system-packages 支持 Ubuntu 24.04）
echo ""
echo "📦 安装核心依赖..."
pip install --break-system-packages -q lancedb numpy pyarrow

# 安装模型推理依赖
echo "📦 安装模型推理依赖..."

# 尝试安装 ONNX Runtime
if pip install --break-system-packages -q onnxruntime 2>/dev/null; then
...
# 安装 Transformers
pip install --break-system-packages -q transformers sentencepiece protobuf

# ⚠️ 重要：AutoModel 需要 PyTorch，必须安装
...
if [ -d "$MODEL_DIR" ] && [ -f "$MODEL_DIR/model.safetensors" ] || [ -f "$MODEL_DIR/pytorch_model.bin" ]; then
    echo "  ✓ 模型已存在，跳过下载"
else
    mkdir -p "$MODEL_DIR"
    python3 << 'EOF'
...
if [ "$USE_ONNX" = true ]; then
    ...
    if [ -f "$ONNX_DIR/model.onnx" ]; then
        ...
        pip install --break-system-packages -q optimum[exporters] 2>/dev/null || {
